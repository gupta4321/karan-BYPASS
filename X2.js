...
try {
  var appStateFile = resolve(join(global.client.mainPath, config.APPSTATEPATH || "x2state.json"));
  ...
  return logger.loader("Can't find the bot's x2state.", "error")
}
      ...
      (async () => {
        ...
        for (const command of listCommand) {
          try {
            ...
            try {
              global.loading(`${cra(`LOADED`)} ${cb(config.name)} success`, "X2 CMD LOAD");
            } catch (err) {
